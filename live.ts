...
export default function LiveCTFScoreboard() {
  const q = useQuery();
  const endpoint = q("endpoint", "https://lappungctf.dev/api/v1/scoreboard");
  ...
  const fetchScoreboard = async () => {
    try {
      ...
      if (!parsed.length) {
        try {
          const resUsers = await fetch(usersEndpoint, { cache: "no-store" });
          if (resUsers.ok) {
            ...
            if (Array.isArray(users) && users.length) {
              parsed = users.map((u, i) => ({
                ...
                name: String(u.name ?? u.display_name ?? `Player ${i + 1}`),
                score: 0,
                pos: i + 1,
              }));
            }
